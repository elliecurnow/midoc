...
server <- function(input, output) {
    ...
    output$descmissdata <- shiny::bindEvent(shiny::renderPrint({
        ...
        midoc::descMissData(y=input$y, covs=input$covs, data=data())
...
Although CRA is valid for our example, we may also wish to perform MI. MI is valid in principle if all partially observed variables are unrelated to missingness, given the (fully observed) imputation model predictors. We should include all other analysis model variables in the imputation model for each partially observed variable, in the form implied by the analysis model, so that the analysis and imputation models are "compatible". 
...
server <- function(input, output) {
  ...
  checkmi <- shiny::reactive({
        testthat::evaluate_promise(
          midoc::checkMI(input$dep,input$preds,input$r_cra,input$mdag))$messages
  })
...
server <- function(input, output) {
    ...
    checkmodspec <- shiny::reactive({
        testthat::evaluate_promise(
          midoc::checkModSpec(input$formula,input$family,data=data(),plot=FALSE))$messages
    })
...
server <- function(input, output) {
    ...
    mimod <- shiny::reactive(
                midoc::checkModSpec(input$formula,input$family,data=data(),
                                    plot=FALSE, message=FALSE))
    
    proposemi <- shiny::reactive({
        testthat::evaluate_promise(
          midoc::proposeMI(mimodobj=mimod(), data=data(),plot=FALSE))$messages
    })
...
server <- function(input, output) {
    ...
    mimod <- shiny::reactive(
                midoc::checkModSpec(input$impformula,input$impfamily,data=data(),
                                      plot=FALSE, message=FALSE))
    
    miprop <- shiny::reactive(
                midoc::proposeMI(mimodobj=mimod(), data=data(),plot=FALSE, message=FALSE))
