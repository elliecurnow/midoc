...
```{r, eval=FALSE, R.options=list(width=80)}
...
exploreDAG(mdag="matage -> bmi7 mated -> matage mated -> bmi7 
           sep_unmeas -> mated sep_unmeas -> r", data=bmi)
```
...
```{r, R.options=list(width=80)}
descMissData(y="bmi7", covs="matage mated pregsize bwt", 
             data=bmi)
```
...
checkMI(dep="bmi7", preds="matage mated pregsize", r_dep="r",
        mdag="matage -> bmi7 mated -> matage mated -> bmi7 
        sep_unmeas -> mated sep_unmeas -> r 
        pregsize -> bmi7 pregsize -> bwtsep_unmeas -> bwt")
```
...
server <- function(input, output) {
    ...
    output$checkmodspec <- shiny::bindEvent(shiny::renderPrint({
      ...
      midoc::checkModSpec(input$formula,input$family,data(),plot=FALSE)
...
mimod <- checkModSpec(formula="bmi7~matage+I(matage^2)+mated+pregsize", 
                      ...
                      data=bmi,
                      message=FALSE)
miprop <- proposeMI(mimod, data=bmi, plotprompt=FALSE)
```
...
server <- function(input, output) {
    ...
    mimod <- shiny::reactive(midoc::checkModSpec(input$formula,input$family,data(),message=FALSE,plot=FALSE))
...
server <- function(input, output) {
    ...
    mimod <- shiny::reactive(midoc::checkModSpec(input$impformula,input$impfamily,data(),message=FALSE,plot=FALSE))
    
    miprop <- shiny::reactive(midoc::proposeMI(mimod(),data(),message=FALSE,plot=FALSE))
