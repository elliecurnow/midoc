...
ui <- fluidPage(
  ...
  # App title ----
  titlePanel("Shiny app to draw the specified directed acyclic graph (DAG)"),
...
server <- function(input, output) {
    ...
    output$mdagplot <- renderPlot(plot(dagitty(mdagspec(),layout=T)))
...
```{r}
checkcra(y="bmi7", covs="matage C", r="R",
         mdag="C -> bmi7 C -> matage matage -> bmi7 C -> R")
```
...
ui <- fluidPage(
  ...
  # App title ----
  titlePanel("Shiny app to check validity of a complete records analysis"),
...
server <- function(input, output) {
  ...
  checkcra(input$y,input$covs,input$r,input$mdag)
...
```{r}
checkmi(dep="bmi7", preds="matage C pregsize", r="R",
...
```{r}
checkmi(dep="bmi7", preds="matage C bwt", r="R",
         mdag="C -> bmi7 C -> matage matage -> bmi7 C -> R C -> bwt pregsize -> bmi7   pregsize -> bwt  U -> bwt  U -> R  U -> C")
```
...
ui <- fluidPage(
  ...
  # App title ----
  titlePanel("Shiny app to check validity of multiple imputation"),
...
server <- function(input, output) {
  ...
  checkmi(input$dep,input$preds,input$r,input$mdag)
