...
**Note** An interactive version of this vignette: *Multiple Imputation DOCtor (midoc) Shiny version* is also available to run locally (you can run this using the `midoc` command `runShinyVignette()`). In the interactive version, you can apply features of `midoc` described here using your own DAG and data.  
