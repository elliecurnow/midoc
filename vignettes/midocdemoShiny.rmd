  ...
  shiny::sidebarLayout(
    ...
    shiny::sidebarPanel(
          shiny::textAreaInput("mdag",
                  ...
                  value = 'mated -> bmi7 mated -> matage matage -> bmi7 mated -> R')
          ),
...
ui <- shiny::fluidPage(
  ...
  shiny::sidebarLayout(
    ...
    shiny::sidebarPanel(
      ...
      shiny::textAreaInput(inputId = "mdag",
                  label = "Directed acyclic graph",
                  value = "mated -> bmi7 mated -> matage matage -> bmi7 mated -> R mated -> bwt pregsize -> bmi7 pregsize -> bwt  U -> bwt  U -> R  U -> mated")
    ),
...
Since we know CRA is valid in principle for our worked example, we can use the complete records in the `bmi` dataset to explore the specification of relationships between BMI at age 7 years and the predictors (just including the analysis model variables, maternal age and maternal education) in our imputation model for BMI at age 7 years. 
...
```{r, echo=FALSE, out.width="400px", out.height="300px"}
fit <- stats::glm(formula=bmi7~matage+mated, family=gaussian(identity), data=bmi)
