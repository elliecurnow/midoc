...
**Note** This is an interactive version of the `midoc` vignette: *Multiple Imputation DOCtor (midoc)*. In this version, you can apply features of `midoc` using your own DAG and data.  
...
**You can list the missing data patterns in your own dataset using a Shiny app, by uploading your dataset (in CSV format) and editing the text in the box below. Note that if you do not upload a file, the default `bmi` dataset will be analysed. Any "in-process" error messages can be ignored (i.e. while you update the input values).**
...
server <- function(input, output) {
    ...
    output$descmissdata <- shiny::renderPrint({
...
**You can draw your own DAG using a Shiny app, by editing the text in the box below. Any "in-process" error messages can be ignored (i.e. while you update the input values).**
...
**You can explore whether relationships in your own dataset are consistent with your proposed mDAG using a Shiny app, by uploading your dataset (in CSV format) and editing the text in the box below. Note that if you do not upload a file, the default `bmi` dataset will be analysed. Any "in-process" error messages can be ignored (i.e. while you update the input values).**
...
**You can check whether CRA is valid for your own DAG and analysis model using a Shiny app, by editing the text in the box below. Any "in-process" error messages can be ignored (i.e. while you update the input values).**
...
server <- function(input, output) {
  ...
  midoc::checkCRA(input$y,input$covs,input$r_cra,input$mdag)
...
We assume that pregnancy size is a cause of BMI at age 7 years, but not its missingness, whereas we assume birth weight is related to both BMI at 7 years (via pregnancy size) and its missingness (via U). We will now add these variables to our mDAG. Below, we have shown our updated mDAG. 
...
server <- function(input, output) {
  ...
  midoc::checkMI(input$dep,input$preds,input$r_dep,input$mdag)
...
**You can explore whether relationships in your own dataset are consistent with your proposed model using a Shiny app, by uploading your dataset (in CSV format) and editing the text in the box below. Note that the diagnostic plot is not displayed (i.e. `modplot` is set to `FALSE`). Further note that if you do not upload a file, the default `bmi` dataset will be analysed. Any "in-process" error messages can be ignored (i.e. while you update the input values).**
...
server <- function(input, output) {
    ...
    output$checkmodspec <- shiny::renderPrint({

    midoc::checkModSpec(input$formula,input$family,data(),modplot=FALSE)
...
We have explored both the validity of MI in principle, using our mDAG, and the specification of our imputation model, based on our observed data. We will now use the `midoc` function `proposeMI` to choose the best options when performing MI using [mice](https://doi.org/10.18637/jss.v045.i03). We will first save our chosen imputation model (i.e. specifying a quadratic relationship between BMI at age 7 years and maternal age) as a `mimod` object. We will then use this, along with our dataset, to construct our call of the "mice" function. Note the `printprompt = FALSE` option of `proposeMI` is used to suppress user prompts to the R Console before displaying the second plot - this is useful in this vignette, but we recommend using the default value of `TRUE` in regular use of `proposeMI`. Note we will also save our proposed "mice" call as a `miprop` object, to be used later. 
...
miprop <- proposeMI(mimod, data=bmi, printprompt=FALSE)
```

**You can explore the optimal "mice" options for your own dataset, based on your specified imputation model, using a Shiny app, by uploading your dataset (in CSV format) and editing the text in the box below. Note that the diagnostic plots are not displayed (i.e. `diagplot` is set to `FALSE`). Further note that if you do not upload a file, the default `bmi` dataset will be analysed. Any "in-process" error messages can be ignored (i.e. while you update the input values).**
...
server <- function(input, output) {
    ...
    mimod <- reactive(midoc::checkModSpec(input$formula,input$family,data(),message=FALSE,modplot=FALSE))
    
    output$proposemi <- shiny::renderPrint({

    midoc::proposeMI(mimod(),data(),diagplot=FALSE)
...
**You can perform multiple imputation using "mice" for your own dataset, based on your specified imputation and analysis models, using a Shiny app, by uploading your dataset (in CSV format) and editing the text in the box below. Note that if you do not upload a file, the default `bmi` dataset will be analysed. Any "in-process" error messages can be ignored (i.e. while you update the input values).**
...
server <- function(input, output) {
    ...
    mimod <- reactive(midoc::checkModSpec(input$impformula,input$impfamily,data(),message=FALSE,modplot=FALSE))
    
    miprop <- reactive(midoc::proposeMI(mimod(),data(),message=FALSE,diagplot=FALSE))
    
    output$domimice <- shiny::renderPrint({
