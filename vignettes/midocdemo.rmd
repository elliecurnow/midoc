...
Our next step is to determine whether complete records analysis (CRA) is a valid strategy, using our mDAG. Remember that, in general, CRA will be valid if the analysis model outcome is unrelated to the complete records indicator, conditional on the analysis model covariates (in special cases, depending on the type of analysis model, this rule can be relaxed - here, we will consider the general setting without making any assumptions about the fitted model) [3](https://doi.org/10.1093/ije/dyz032). 

We will use the `midoc` function `checkCRA` applied to our mDAG to check whether complete records analysis (CRA) is valid, specifying our outcome (`y`), covariates, i.e. our independent variables, (`covs`), complete records indicator (`r`), and mDAG (`mdag`).

Suppose we decide to estimate the unadjusted association between BMI at age 7 years and maternal age, without including our confounder maternal education in the model, using CRA. The results are shown below (note, tap "Show" to reveal the code used to create these results).   
...
We can see that CRA would not be valid (we can also tell this by inspecting our DAG: there is an open path from `bmi7` to `R` via `mated` and `U` if we only condition on `matage`). `checkCRA` suggests that CRA would be valid if we included `mated`, or `mated` and `U`, in the analysis model. Since `U` is unmeasured, the only available option is to include `mated` in the analysis model. In this particular setting, this seems sensible since `mated` is a confounder. If we add `mated` to the model and re-run `checkCRA`, as below, we see that CRA is now valid, in principle.
